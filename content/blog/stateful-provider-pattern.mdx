...
context mixed with state" which still doesn't do justice to this pattern.  So, I gave it a name!  I call it the `Stateful Provider 
Pattern` and you should too!
...
The most recent iteration of the [Context API]([https://reactjs.org/docs/context.html](https://reactjs.org/docs/context.html)) (introduced
...
[Observer Pattern](h[ttps://en.wikipedia.org/wiki/Observer_pattern](https://en.wikipedia.org/wiki/Observer_pattern)) popularized by the
classic [Gang of Four]([https://martinfowler.com/bliki/GangOfFour.html](https://martinfowler.com/bliki/GangOfFour.html)) book.  Don't 
...
values connect to the provider (with the `useContext` hook) so they can access its values. These components are considered *consumers* of 
the `Provider`.
...
Now that we know a bit more about the Context API and the `useState` hook, it's time to talk about how we weave them together into a `Stateful
Provider`.  Thinking back to the section about dependency injection, what if the dependency isn't static?  What if it needs to change over time?
...
`Provider` to allow the `value` to be changed.  Let's explore that a bit with a working example:

<iframe src="https://codesandbox.io/embed/react-context-demo-7jjge?fontsize=14&hidenavigation=1&theme=dark"
...
<iframe src="https://codesandbox.io/embed/stateful-provider-demo-8fee2?fontsize=14&hidenavigation=1&theme=dark"
style={{width:'100%', height:'500px', border:0, borderRadius: '4px', overflow:'hidden'}}
